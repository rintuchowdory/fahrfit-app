...
import AdminDraftsPanel from "@/components/AdminDraftsPanel";
import MediaQuestionPlayer from "@/components/MediaQuestionPlayer";
import { buildExamSummary } from "../../../shared/exam";
...
type Question = { id: number; topic: string; prompt: string; context: string; options: string[]; correct: number[]; explanation: string; difficulty: string; weight: number; mediaUrl?: string | null; mediaType?: "image" | "video" | null; thumbnailUrl?: string | null; mediaAlt?: string | null };
...
const questions: Question[] = [
  { id: 1, topic: "Vorfahrt", weight: 5, prompt: "Sie nähern sich einer Kreuzung ohne Verkehrszeichen. Was gilt grundsätzlich?", context: "Kreuzung · Sicht frei · keine Ampel", options: ["Die Regel „rechts vor links“", "Das größere Fahrzeug hat Vorfahrt", "Wer zuerst hupt, fährt zuerst"], correct: [0], explanation: "An Kreuzungen und Einmündungen ohne besondere Regelung gilt grundsätzlich rechts vor links.", difficulty: "Grundlagen" },
  { id: 2, topic: "Abstand", weight: 3, prompt: "Warum ist ein ausreichender Sicherheitsabstand besonders wichtig?", context: "Trockene Fahrbahn · dichter Verkehr", options: ["Damit die Reaktions- und Bremszeit berücksichtigt wird", "Damit andere nicht überholen können", "Damit der Motor weniger Kraftstoff verbraucht"], correct: [0], explanation: "Der Abstand schafft Reaktionsraum und reduziert das Risiko eines Auffahrunfalls.", difficulty: "Grundlagen" },
  { id: 3, topic: "Gefahrenlehre", weight: 4, prompt: "Was sollten Sie bei plötzlich auftretendem Nebel zuerst tun?", context: "Sichtweite nimmt schnell ab · Landstraße", options: ["Geschwindigkeit vorsichtig anpassen", "Sofort auf die Gegenfahrbahn wechseln", "Dicht auf das vorausfahrende Fahrzeug auffahren"], correct: [0], explanation: "Bei eingeschränkter Sicht muss die Geschwindigkeit so angepasst werden, dass die Strecke übersehbar bleibt.", difficulty: "Aufmerksamkeit" },
  { id: 4, topic: "Verkehrszeichen", weight: 2, prompt: "Was kündigt ein dreieckiges Warnzeichen in der Regel an?", context: "Verkehrszeichen · roter Rand · weiße Fläche", options: ["Eine Gefahrstelle", "Ein verbindliches Parkverbot", "Das Ende aller Beschränkungen"], correct: [0], explanation: "Dreieckige Zeichen mit rotem Rand warnen in der Regel vor Gefahrenstellen.", difficulty: "Grundlagen" },
  { id: 5, topic: "Geschwindigkeit", weight: 3, prompt: "Welche Aussage unterstützt vorausschauendes Fahren?", context: "Unübersichtliche Straße · Kinder am Fahrbahnrand", options: ["Frühzeitig langsamer werden und bremsbereit sein", "Nur auf die eigene Spur schauen", "Erst reagieren, wenn jemand die Fahrbahn betritt"], correct: [0], explanation: "Vorausschauendes Fahren bedeutet, mögliche Gefahren früh zu erkennen und die Geschwindigkeit rechtzeitig anzupassen.", difficulty: "Praxis" },
  { id: 6, topic: "Gefahrenlehre", weight: 5, prompt: "Welche Verhaltensweisen helfen bei eingeschränkter Sicht?", context: "Dämmerung · wechselnde Sichtverhältnisse", options: ["Geschwindigkeit anpassen", "Abstand verkürzen", "Beleuchtung kontrollieren"], correct: [0, 2], explanation: "Bei eingeschränkter Sicht sind eine angepasste Geschwindigkeit und eine passende Beleuchtung wichtig. Mehr Abstand schafft zusätzlich Sicherheitsreserven.", difficulty: "Mehrfachauswahl" },
];
...
export default function Home() {
  ...
  const [examPenaltyPoints, setExamPenaltyPoints] = useState(0);
  const [examFivePointErrors, setExamFivePointErrors] = useState(0);
  ...
  const recentSessionsQuery = trpc.learning.recentSessions.useQuery(undefined, { enabled: isAuthenticated, staleTime: 30_000 });
  ...
  const [adminTab, setAdminTab] = useState<"editor" | "drafts">("editor");
  ...
  const catalog = (questionQuery.data?.length ? questionQuery.data.map((item) => ({ id: item.id, topic: String(item.topicId), prompt: item.prompt, context: item.mediaAlt || "Klasse B · Verkehrssituation", options: item.options.map(option => option.text), correct: item.options.filter(option => option.isCorrect === 1).map(option => item.options.indexOf(option)), explanation: item.explanation, difficulty: item.difficulty, weight: item.weight ?? 3, mediaUrl: item.mediaUrl, mediaType: item.mediaType, thumbnailUrl: item.thumbnailUrl, mediaAlt: item.mediaAlt })) : questions);
  const liveErrorIds = errorIdsQuery.data ?? JSON.parse(localStorage.getItem("fahrfit-error-ids") || "[]") as number[];

  const startSession = async (nextMode: "learn" | "errors" | "exam", topicId?: number | null) => {
    const source = catalog.length ? catalog : questions;
    const topicName = topicId ? (topicQuery.data?.find(topic => topic.id === topicId)?.name ?? topics[topicId - 1]?.name) : undefined;
    const topicPool = (nextMode === "learn" || nextMode === "errors") && topicId ? source.filter(q => questionQuery.data?.length ? q.topic === String(topicId) : q.topic === topicName) : source;
    const pool = nextMode === "errors" ? topicPool.filter(q => liveErrorIds.includes(q.id)) : topicPool;
    const selectedQuestions = nextMode === "errors" ? pool : nextMode === "exam" ? source : pool.slice(0, 4);
    setSessionQuestions(selectedQuestions); setActiveQuestion(0); setSelected([]); setAnswered(false); setSessionScore(0); setExamPenaltyPoints(0); setExamFivePointErrors(0); setMode(nextMode);
  ...
  const submitAnswer = () => {
    ...
    setIsCorrect(correct); setAnswered(true); if (correct) setSessionScore(score => score + 1); else { if (mode === "exam") { setExamPenaltyPoints(points => points + current.weight); if (current.weight === 5) setExamFivePointErrors(count => count + 1); } const nextErrors = Array.from(new Set([...liveErrorIds, current.id])); localStorage.setItem("fahrfit-error-ids", JSON.stringify(nextErrors)); }
    if (isAuthenticated && sessionIdRef.current) submitAnswerMutation.mutate({ sessionId: sessionIdRef.current, questionId: current.id, selectedOptionIds: selected.map(index => index + 1), isCorrect: correct, mistakePoints: correct ? 0 : mode === "exam" ? current.weight : 1 });
  };
  ...
  const examHistory = useMemo(() => (recentSessionsQuery.data ?? []).filter(session => session.mode === "exam").slice().reverse().slice(-8), [recentSessionsQuery.data]);
  const latestExam = examHistory.at(-1);
  const readinessScore = calculateReadinessScore(latestExam?.mistakePoints);
  const readinessLabel = getReadinessLabel(readinessScore, examHistory.length);
  const dashboardProgressLabel = latestExam ? readinessLabel : "Gesamtfortschritt";

  const handleAdminDrop = (event: React.DragEvent<HTMLLabelElement>) => { event.preventDefault(); setIsDraggingMedia(false); void handleAdminFile(event.dataTransfer.files?.[0]); };
  ...
  if (["learn", "errors", "exam"].includes(mode)) {
    ...
    if (sessionQuestions.length === 0) return <div className="app-shell"><Sidebar mode={mode} setMode={setMode} nav={nav} mobileNav={mobileNav} setMobileNav={setMobileNav} /><main className="main-content learning-main"><div className="learning-toolbar"><button className="back-link" onClick={() => setMode("dashboard")}><ChevronLeft size={18} /> Zur Übersicht</button></div><div className="empty-session-state"><CircleAlert size={32} /><h2>Keine Fragen gefunden</h2><p>Für dieses Thema oder deine Fehlerliste sind aktuell keine Fragen verfügbar.</p><Button className="primary-action" onClick={() => setMode("dashboard")}>Zurück zur Übersicht</Button></div></main></div>;
    return <div className="app-shell"><Sidebar mode={mode} setMode={setMode} nav={nav} mobileNav={mobileNav} setMobileNav={setMobileNav} />
      <main className="main-content learning-main">
        ...
        <div className="learning-toolbar"><button className="back-link" onClick={() => setMode("dashboard")}><ChevronLeft size={18} /> Zur Übersicht</button><div className="session-meta"><span>{isExam ? "Prüfungssimulation" : mode === "errors" ? "Deine Fehler" : "Thema üben"}</span><span className="dot-separator">•</span><span>{current.topic}</span></div>{(mode === "learn" || mode === "errors") && <div className="topic-filter"><label htmlFor="session-topic">Thema</label><select id="session-topic" value={selectedTopicId ?? ""} onChange={event => setSelectedTopicId(Number(event.target.value) || null)}><option value="">Alle Themen</option>{(topicQuery.data ?? topics.map((topic, index) => ({ id: index + 1, name: topic.name }))).map(topic => <option key={topic.id} value={topic.id}>{topic.name}</option>)}</select><button onClick={() => startSession(mode === "errors" ? "errors" : "learn", selectedTopicId)}>Anwenden</button></div>}<span className="session-count">Frage {activeQuestion + 1} von {sessionQuestions.length}</span></div>
        <div className="player-progress-summary"><div className="progress-track"><div className="progress-fill" style={{ width: `${progress}%` }} /></div><div className="score-summary"><span><strong>{progress}%</strong> abgeschlossen</span><span><strong>{sessionScore}</strong> richtig</span><span><strong>{Math.max(0, activeQuestion - sessionScore + (answered && !isCorrect ? 1 : 0))}</strong> zu wiederholen</span></div></div>
        <section className="question-layout">
          <div className="question-copy"><div className="eyebrow"><span className="status-dot" /> {current.difficulty}</div><h1>{current.prompt}</h1><p className="question-context"><CircleAlert size={17} /> {current.context}</p>{current.mediaUrl && <div className="question-media">{current.mediaType === "video" ? <MediaQuestionPlayer src={current.mediaUrl} poster={current.thumbnailUrl} alt={current.mediaAlt || "Verkehrssituation"} /> : <img src={current.mediaUrl} alt={current.mediaAlt || "Verkehrssituation"} onError={event => { event.currentTarget.alt = "Medium konnte nicht geladen werden"; }} />}</div>}<div className="answer-list">{current.options.map((option, index) => <button key={option} className={`answer-option ${selected.includes(index) ? "selected" : ""} ${answered && !isExam && current.correct.includes(index) ? "correct" : ""} ${answered && !isExam && selected.includes(index) && !current.correct.includes(index) ? "wrong" : ""}`} onClick={() => chooseAnswer(index)}><span className="answer-letter">{String.fromCharCode(65 + index)}</span><span>{option}</span>{answered && !isExam && current.correct.includes(index) && <Check className="answer-icon" size={19} />}{answered && !isExam && selected.includes(index) && !current.correct.includes(index) && <X className="answer-icon" size={19} />}</button>)}</div>{selected.length > 0 && !answered && <div className="answer-feedback" aria-live="polite"><Check size={16} /><span>{selected.length === 1 ? "1 Antwort ausgewählt" : `${selected.length} Antworten ausgewählt`}</span><small>{isExam ? "Deine Auswahl wird am Ende bewertet." : "Du kannst die Auswahl noch ändern."}</small></div>}<div className="question-actions">{!answered ? <Button className="primary-action" disabled={!selected.length} onClick={submitAnswer}>Antwort prüfen <ArrowRight size={18} /></Button> : <Button className="primary-action" onClick={nextQuestion}>{activeQuestion === sessionQuestions.length - 1 ? "Einheit beenden" : "Nächste Frage"} <ArrowRight size={18} /></Button>}</div></div>
  ...
  if (mode === "exam-result") {
    const examSummary = buildExamSummary({ totalQuestions: sessionQuestions.length, correctAnswers: sessionScore, penaltyPoints: examPenaltyPoints, fivePointErrors: examFivePointErrors });
    const finalScore = examSummary.correctAnswers;
    const passed = examSummary.passed;
    return <div className="app-shell"><Sidebar mode={mode} setMode={setMode} nav={nav} mobileNav={mobileNav} setMobileNav={setMobileNav} /><main className="main-content"><header className="topbar"><div><p className="breadcrumb">FahrFit <span>/</span> Prüfung</p><h1>Deine Prüfung ist beendet.</h1><p className="muted">Hier ist deine Auswertung für Klasse B.</p></div><div className="topbar-actions"><div className="avatar">A</div></div></header><section className="result-hero"><div className={`result-icon ${passed ? "success" : "error"}`}>{passed ? <Check size={27} /> : <X size={27} />}</div><span className={`feedback-kicker ${passed ? "success-text" : "error-text"}`}>{passed ? "Bestanden" : "Weiter üben"}</span><h2>{passed ? "Prüfung bestanden" : "Prüfung nicht bestanden"}</h2><p>{finalScore} von {sessionQuestions.length} Fragen richtig · {examPenaltyPoints} Fehlerpunkte{examFivePointErrors ? ` · ${examFivePointErrors}× 5-Punkte-Frage falsch` : ""}. {passed ? "Stark gelöst. Festige jetzt noch deine letzten Unsicherheiten." : "Kein Problem. Deine Fehler zeigen dir genau, wo du weiterlernen kannst."}</p></section><section className="result-grid"><div className="result-stat"><span className="eyebrow">ERGEBNIS</span><strong>{examPenaltyPoints}<small> Pkt.</small></strong><p>Fehlerpunkte · maximal 10</p></div><div className="result-stat"><span className="eyebrow">FEHLERANALYSE</span><strong>{examSummary.wrongAnswers}</strong><p>Fragen für dein Fehlertraining</p></div><div className="result-stat"><span className="eyebrow">EMPFEHLUNG</span><strong className="recommendation">{passed ? "Festigen" : "Nachlernen"}</strong><p>{passed ? "Wiederhole die unsicheren Themen." : "Starte mit deinen Fehlerfragen."}</p></div></section><p className="exam-rule-note">Klasse B: Bestanden wird mit höchstens 10 Fehlerpunkten. Zwei falsch beantwortete Fragen mit je 5 Punkten führen unabhängig von der Gesamtsumme zum Nichtbestehen.</p><div className="result-actions"><Button className="primary-action" onClick={() => startSession("errors")}>Fehler lernen <ArrowRight size={18} /></Button><button className="text-action" onClick={() => startSession("exam")}>Neue Prüfung <RotateCcw size={16} /></button></div></main></div>;
  }

  if (mode === "progress") {
    const historyMax = Math.max(10, ...examHistory.map(session => session.mistakePoints));
    const chartPoints = examHistory.map((session, index) => `${index * (100 / Math.max(1, examHistory.length - 1))},${100 - (session.mistakePoints / historyMax) * 82 - 9}`).join(" ");
    return <div className="app-shell"><Sidebar mode={mode} setMode={setMode} nav={nav} mobileNav={mobileNav} setMobileNav={setMobileNav} /><main className="main-content"><header className="topbar"><div><p className="breadcrumb">FahrFit <span>/</span> Klasse B</p><h1>Dein Fortschritt.</h1><p className="muted">Jeder Lernschritt bringt dich näher an den Prüfungstag.</p></div><div className="topbar-actions"><div className="avatar">A</div></div></header><section className="progress-overview"><div><span className="eyebrow">GESAMTFORTSCHRITT</span><strong>{progressQuery.data?.byTopic?.length ? Math.round(progressQuery.data.byTopic.reduce((sum, item) => sum + item.percent, 0) / progressQuery.data.byTopic.length) : topicAverage}%</strong><p>über alle Klasse-B-Themen</p></div><div className="big-progress"><div style={{ width: `${topicAverage}%` }} /></div></section><section className="analytics-grid"><div className="analytics-panel penalty-chart-panel"><div className="analytics-heading"><div><span className="eyebrow">PRÜFUNGSSIMULATIONEN</span><h2>Fehlerpunkte sinken.</h2><p>Weniger Punkte bedeuten mehr Prüfungssicherheit.</p></div><span className="trend-chip">{latestExam ? `${latestExam.mistakePoints} Pkt. zuletzt` : "Noch keine Daten"}</span></div>{examHistory.length > 1 ? <div className="penalty-chart"><div className="chart-y-labels"><span>{historyMax}</span><span>{Math.round(historyMax / 2)}</span><span>0</span></div><div className="chart-area"><div className="chart-grid-line top" /><div className="chart-grid-line mid" /><div className="chart-grid-line bottom" /><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label="Fehlerpunkteverlauf"><polyline points={chartPoints} fill="none" stroke="#6f5bec" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round" />{examHistory.map((session, index) => <circle key={session.id} cx={index * (100 / Math.max(1, examHistory.length - 1))} cy={100 - (session.mistakePoints / historyMax) * 82 - 9} r="2.7" fill="#fff" stroke="#6f5bec" strokeWidth="2" vectorEffect="non-scaling-stroke" />)}</svg><div className="chart-x-labels">{examHistory.map((session, index) => <span key={session.id}>#{index + 1}</span>)}</div></div></div> : <div className="analytics-empty"><img src="/manus-storage/progress-reference_e9b4a1a1.webp" alt="Lernfortschritt Illustration" /><span>Starte deine erste Prüfungssimulation, um den Verlauf zu sehen.</span></div>}</div><div className="analytics-panel readiness-panel"><div className="readiness-orbit" style={{ background: `conic-gradient(#5ce0bb ${readinessScore}%, rgba(255,255,255,.13) 0)` }}><div><strong>{readinessScore}%</strong><span>Reife</span></div></div><span className="eyebrow">PRÜFUNGSREIFE</span><h2>{readinessLabel}</h2><p>{latestExam ? `Basierend auf ${examHistory.length} Simulation${examHistory.length === 1 ? "" : "en"} und deinem letzten Fehlerpunktestand.` : "Deine ersten Simulationen bauen die persönliche Basis auf."}</p><button onClick={() => startSession("exam")}>Neue Simulation <ArrowRight size={16} /></button></div></section><div className="section-heading"><div><span className="eyebrow">THEMENSTATUS</span><h2>Wissen wird sicher.</h2></div></div><div className="topic-grid">{topics.map(topic => <div className="topic-card progress-topic" key={topic.name}><div className="topic-icon" style={{ backgroundColor: `${topic.color}18`, color: topic.color }}>{topic.icon}</div><div className="topic-main"><div className="topic-name"><span>{topic.name}</span><span>{topic.percent}%</span></div><div className="topic-track"><div style={{ width: `${topic.percent}%`, backgroundColor: topic.color }} /></div></div></div>)}</div></main></div>;
  }
  ...
  return <div className="app-shell"><Sidebar mode={mode} setMode={setMode} nav={nav} mobileNav={mobileNav} setMobileNav={setMobileNav} />
    <main className="main-content"><header className="topbar"><div><p className="breadcrumb">FahrFit <span>/</span> Klasse B</p><h1>Guten Morgen, {displayName}.</h1><p className="muted">Dein nächster Schritt zum sicheren Prüfungstag.</p></div><div className="topbar-actions"><button className="streak-pill"><span>✦</span> 7 Tage Lernserie</button><div className="avatar">{displayName.slice(0, 1).toUpperCase()}</div></div></header>
      ...
      <section className="visual-learning-strip"><div className="visual-card visual-card-main"><img src="/manus-storage/practice-reference_a8f943c2.webp" alt="Illustrierte Fahrschul-Übungssituation" /><div className="visual-card-copy"><span className="eyebrow light">VISUELL LERNEN</span><h2>Üben wie am Prüfungstag.</h2><p>Fragen, Verkehrssituationen und dein nächster Lernschritt an einem Ort.</p><button onClick={() => startSession("exam")}>Simulation starten <ArrowRight size={16} /></button></div></div><div className="visual-card visual-card-side"><img src="/manus-storage/progress-reference_e9b4a1a1.webp" alt="Illustrierte Lernfortschrittsansicht" /><div><span className="eyebrow">DEIN FORTSCHRITT</span><strong>{latestExam ? readinessScore : topicAverage}%</strong><p>{dashboardProgressLabel}</p><button onClick={() => setMode("progress")}>Details ansehen <ArrowRight size={16} /></button></div></div></section><div className="section-heading"><div><span className="eyebrow">DEIN LERNWEG</span><h2>Themen im Überblick</h2></div><button className="view-all" onClick={() => setMode("progress")}>Alle anzeigen <ArrowRight size={16} /></button></div><div className="topic-grid">{topics.map(topic => <button className="topic-card" key={topic.name} onClick={() => { setSessionQuestions(questions.filter(question => question.topic === topic.name).length ? questions.filter(question => question.topic === topic.name) : questions.slice(0, 4)); setActiveQuestion(0); setAnswered(false); setSelected([]); setMode("learn"); }}><div className="topic-icon" style={{ backgroundColor: `${topic.color}18`, color: topic.color }}>{topic.icon}</div><div className="topic-main"><div className="topic-name"><span>{topic.name}</span><span>{topic.percent}%</span></div><div className="topic-track"><div style={{ width: `${topic.percent}%`, backgroundColor: topic.color }} /></div></div><ArrowRight size={17} className="topic-arrow" /></button>)}</div>
