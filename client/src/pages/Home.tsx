...
export default function Home() {
  ...
  const startSession = async (nextMode: "learn" | "errors" | "exam", topicId?: number | null) => {
    ...
    const topicPool = nextMode === "learn" && topicId ? source.filter(q => questionQuery.data?.length ? q.topic === String(topicId) : q.topic === topicName) : source;
  ...
  if (["learn", "errors", "exam"].includes(mode)) {
    ...
    return <div className="app-shell"><Sidebar mode={mode} setMode={setMode} nav={nav} mobileNav={mobileNav} setMobileNav={setMobileNav} />
      <main className="main-content learning-main">
        ...
        <div className="learning-toolbar"><button className="back-link" onClick={() => setMode("dashboard")}><ChevronLeft size={18} /> Zur Übersicht</button><div className="session-meta"><span>{isExam ? "Prüfungssimulation" : mode === "errors" ? "Deine Fehler" : "Thema üben"}</span><span className="dot-separator">•</span><span>{current.topic}</span></div>{mode === "learn" && <div className="topic-filter"><label htmlFor="learn-topic">Thema</label><select id="learn-topic" value={selectedTopicId ?? ""} onChange={event => setSelectedTopicId(Number(event.target.value) || null)}><option value="">Alle Themen</option>{(topicQuery.data ?? topics.map((topic, index) => ({ id: index + 1, name: topic.name }))).map(topic => <option key={topic.id} value={topic.id}>{topic.name}</option>)}</select><button onClick={() => startSession("learn", selectedTopicId)}>Anwenden</button></div>}<span className="session-count">Frage {activeQuestion + 1} von {sessionQuestions.length}</span></div>
